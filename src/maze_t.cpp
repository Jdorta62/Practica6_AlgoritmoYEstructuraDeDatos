...
bool maze_t::solve_(const int i, const int j) {
  ...
  if (is_ok_((i + i_d[0]), (j + j_d[0]))) {
    if (solve_((i + i_d[0]), (j + j_d[0]))) {
      matrix_(i,j) = PATH_ID;
      pair_t<int> coordinate(i,j);
      dll_node_t<pair_t<int>>* coordinate_node = new dll_node_t<pair_t<int>>(coordinate);
      solution_.push_front(coordinate_node);
      return true;
    }
  }
